...
data <- data |>
  mutate(across(c(
    Votes, `%vote`, HOFm, HOFs, Yrs, WAR, WAR7, JAWS, Jpos, G, AB, R, H, HR, RBI, SB, BB, BA, OBP, SLG, OPS, `OPS+`, W, L, ERA, `ERA+`, WHIP, G.1, GS, SV, IP, H.1, HR.1, BB.1, SO), ~ as.numeric(parse_number(.))))
data <- data |>
...
```{r Make CSV}
write.csv(data_modified, file = "HarshMishrikotkar_hof_2025_clean.csv", row.names = FALSE)
```